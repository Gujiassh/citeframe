...
- **当前 accepted-A2a 边界（模块化双进程，非独立服务）**：API package 仍负责 HTTP/auth/Alembic/schema governance；accepted A2a production 已把 DB-only Research transition 收口到 neutral `citeframe_research_persistence`，Worker default composition 通过 neutral UoW/repository 管理 Session/commit，API 保留 compatibility/external-adapter composition；ingestion 仍在 API orchestrator 与 Worker adapter 的共享 Session/ORM 边界。A2a 已在本地 production `215cd52`、docs `95981a4`、review `eb97adf` 获得独立 `ACCEPT`；三笔提交均未推送，不得描述为已完成远端交付。跟进见 [`docs/architecture/api-worker-boundary-follow-up-2026-08-18.md`](../architecture/api-worker-boundary-follow-up-2026-08-18.md)。
