...
> 2026-08-25 current-state override: A2a/R0 are delivered at `origin/main@8674d4d`. R1 is independently `ACCEPTED (High=0, Medium=0, Low=0)` across start `8674d4d`, historical initial `f4a1d1d(REWORK)`, runtime `473213d`, ledger `652cfd4`, docs `559997d`, delivery truth `80d395d`, and review `5a55489`; PR #22 delivered the complete chain to `origin/main@a616eea1350b095c6f229890d2c47e5010902330` with `6/6` CI. R2 is the only next separately gated module; W1/admission/downstream remain blocked.
...
- Initial reviewed state defined `ResearchWorkerService`, `SessionFactory`, `_ApiPort`,
  ...
  Research persistence boundary and default Worker UoW composition; both were delivered through PR #21.
...
| **Session / commit process owner** | 哪个**进程**创建 Session 并执行 commit/rollback | Delivered A2a/R1 Research: Worker-side neutral UoW; Ingestion: shared Session boundary | R1 delivered by PR #22 at `origin/main@a616eea` with `6/6` CI |
...
### 4.1 当前依赖方向（A2a/R0 delivered；R1 independently accepted and delivered by PR #22）
...
这是批准目标；A2a/R0 已由 PR #21 交付至 `origin/main@8674d4d`。R1 runtime `473213d`、ledger `652cfd4`、docs `559997d`、delivery truth `80d395d` 已由 review `5a55489` 独立 ACCEPT，并由 PR #22 交付至 `origin/main@a616eea`（六项 CI 全绿）。修订设计的独立 Critical 复审已接受 (`High=0`, `Medium=0`, `Low=0`)；A1 已于 2026-08-20 独立 ACCEPT；A1b/A2-foundation 已于 2026-08-21 独立 ACCEPT（follow-up Critical review：High=0、Medium=0、Low=0）；R2 是唯一下一门控模块，W1/admission 继续 blocked，且不授权 schema/API/save/replay/permission 变更。详细的 R1/R2/W1、锁/fencing、per-Run admission、
SSE 与语义 oracle 见
...
Worker-side UoW；ingestion = 共享 Session/ORM。** A2a/R0 已由 PR #21 交付至 `origin/main@8674d4d`；R1 已在 review `5a55489` 独立 ACCEPT，并由 PR #22 交付至 `origin/main@a616eea`（六项 CI 全绿）；API-process HTTP/RPC 仍未授权。
...
- 本 ADR 与下列**正式 SSoT** 同步当前 repair-worktree 事实（schema governance vs
  ...
  historical; A2a/R0 are delivered and R1 is delivered by PR #22 at `origin/main@a616eea` with `6/6` CI:
...
- No schema/API/save/replay/permission/runtime behavior changed; A1 was independently
  ...
  follow-up Critical review (`High=0`, `Medium=0`, `Low=0`). A2a/R0 are delivered; R1 is delivered by PR #22 at `origin/main@a616eea` with `6/6` CI. R2 is the only next gated module; W1 remains blocked.
...
2. Preserve PR #22 delivery at `origin/main@a616eea` with all six CI checks successful; R2 is the next separately gated module.
