...
### Offline deployment-oracle corrections

Review of06b identified two offline acceptance gaps, now repaired with real-model foreign-key projection and source-contract/finish-claim validation. The actual SQLite v4 journal fixture and eight invalid relation/input controls guard the query path; the three original rehashed false acceptances and two additional schema/coverage variants guard semantics. No production journal field, migration, source contract or R2 whitelist changes are part of this repair. The exact product manifest is143 paths.06b service28PASS and clean A2 remain historical evidence; final-head service/deployment CI and independent review are tracked separately.
