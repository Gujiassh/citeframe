...
## PostgreSQL migration oracle maintenance

PR #39 API CI at `31cbb1f` reached the newest migration rollback guard, while the legacy migration oracle still expected the earlier investigation guard. The test now stops at `r2f3a4b5c6d7`, independently proves the investigation guard and unchanged revision/evidence, then upgrades to head and proves the encrypted-configuration guard with unchanged revision, evidence and opaque stored ciphertext. Its existing dump/restore comparison remains; the restored fixture also retains the new table value. No migration implementation or rollback protection changed.

The real PostgreSQL oracle passed locally (**1 passed**) using test credentials and two run-owned random `ai_pdf_asset_migration_` / `ai_pdf_asset_restore_` databases. A local runner recorded created names and rejected any drop/session termination outside that exact created set; both databases were removed after the run. The user database was not used. This validates disposable-fixture dump/restore; restoration of the user's pre-migration backup remains untested.

## Exact integrated product admission maintenance

The next r800 CI run (`35979517136`, job `107567805248`) reached the exact product-delta verifier and correctly rejected changes absent from the older manifest. Updated only `specs/v5/worker-layout/integrated-product-delta.json`: **127 historical entries unchanged**, **16 changed entries retain their complete previous record as inherited provenance**, and **80 new entries** point to exact source commits/blobs from reviewed PR #37/#39 changes and the separately reviewed migration-oracle repair. No unknown source path, removed admission, product-source modification or guard relaxation was included. Each changed entry uses the commit that last changed its actual bytes; no wholesale current-head admission was used.

All eight historical functional-oracle entries remain in their original order; four additional oracles cover the upload queue, owner configuration and both generation protocols, reindex/question/restart/drift recovery, transport/concurrency and the two rollback guards. `verify_product_delta` passed for all **223 exact product paths** against `8e65033`. `uv run --project apps/api pytest infra/testing/test_r800_product_delta.py infra/testing/test_r800_docker_smoke.py -q --disable-warnings --tb=short`: **13 passed, 5 subtests passed**, including rejection of unknown, changed, missing and duplicate admissions. Independent exact-admission review is required before this manifest repair is pushed.
