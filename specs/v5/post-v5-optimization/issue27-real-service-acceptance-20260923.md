...
## Explicit retry Step delta

The current-candidate comparator reports retryStepErrorDelta independently of R2 publication deltas. In the fixed transitions fixture, exactly one same-run/same-step sequence has attempt1 abandoned with lease_expired, its abandonment/queue events, and a successfully leased running attempt2 with matching frozen input. Only the current Step error_code/error_message change from the old expired-attempt values to null. Original raw reports remain intact; attempt/event history and all other fields retain strict comparison. Eight coherent raw/projection negative controls reject unclaimed clearing, changed historical attempt/event errors, one-field clearing, foreign Step association, wrong input, erased new failure and an unrelated Step field. The original historical fixture is unchanged; rawEqual remains false.
