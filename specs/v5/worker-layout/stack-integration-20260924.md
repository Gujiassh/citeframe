...
### PR30 offline-oracle review repair

The first142-path candidate06b had two independently reproduced offline defects: querying nonexistent journal owner columns, and accepting rehashed fabricated inspection content. Journal ownership is now checked through actual Step/snapshot/attempt foreign-key facts; raw journal columns are retained unchanged. Production `validate_investigation` supplies closed-schema, source quote, condition and coverage checks; finish claim IDs are separately bound to inspect claims. A real SQLite publication path adds10 model-projection regressions; five rehashed semantic controls and nullable Step-input coverage supplement the existing suite. This adds one explicitly sourced test path (143 total); production model/behavior, R2 comparison allowances and frozen historical data are unchanged. The new fixed SHA requires new CI and Hubble re-review.
