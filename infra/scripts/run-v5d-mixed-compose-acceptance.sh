#!/usr/bin/env bash
# V5-D D-G6 empty-target Compose backup/restore for mixed PDF+Image+Markdown.
#
# Reuses compose.deploy.yml + compose.v5b.yml, shared backup/restore contracts,
# mixed seed + restore CLIs. Does not change API/Worker/Web business contracts.
# Never claims realModelQualityPassed or userValuePassed.
...
usage() {
  cat <<'EOF'
Usage: run-v5d-mixed-compose-acceptance.sh --output-dir PATH [--project citeframe-v5d-NAME]

Builds API/Worker/Web images, seeds one mixed PDF+Image+Markdown workspace,
runs production-start Playwright before backup, performs empty-deployment
restore, verifies mixed semantic identity, replays browser after restore, and
removes the isolated Compose project and images.
EOF
}
...
if [[ -z "$COMPOSE_PROJECT" ]]; then
  COMPOSE_PROJECT="citeframe-v5d-$RUN_TOKEN"
fi
if [[ ! "$COMPOSE_PROJECT" =~ ^citeframe-v5d-[a-z0-9][a-z0-9_-]*$ ]]; then
  printf 'v5d_project_must_be_isolated project=%s\n' "$COMPOSE_PROJECT" >&2
  exit 2
fi
OUTPUT_DIR=$(realpath -m "$OUTPUT_DIR")
if [[ -e "$OUTPUT_DIR" ]]; then
  printf 'v5d_output_exists path=%s\n' "$OUTPUT_DIR" >&2
  exit 1
fi
mkdir -m 700 -p "$OUTPUT_DIR"
mkdir -m 700 -p "$OUTPUT_DIR/browser-before" "$OUTPUT_DIR/browser-after"

POSTGRES_PASSWORD="V5D-db-$RUN_TOKEN"
MINIO_ROOT_USER="v5dminio${RANDOM}"
MINIO_ROOT_PASSWORD="V5D-minio-$RUN_TOKEN"
API_INTERNAL_TOKEN="V5D-internal-token-$RUN_TOKEN"
SESSION_SECRET="V5D-session-secret-$RUN_TOKEN-long-enough"
OPENAI_API_KEY="V5D-private-no-network-$RUN_TOKEN"
BROWSER_EMAIL="v5d-browser-$RUN_TOKEN@example.com"
BROWSER_PASSWORD="V5D-browser-password-$RUN_TOKEN-long"
...
API_IMAGE="ai-pdf-workspace-api:v5d-$RUN_TOKEN"
WORKER_IMAGE="ai-pdf-workspace-worker:v5d-$RUN_TOKEN"
WEB_IMAGE="ai-pdf-workspace-web:v5d-$RUN_TOKEN"
...
write_redacted_compose_config() {
  compose config | \
    V5D_SECRET_POSTGRES="$POSTGRES_PASSWORD" \
    V5D_SECRET_MINIO_USER="$MINIO_ROOT_USER" \
    V5D_SECRET_MINIO_PASSWORD="$MINIO_ROOT_PASSWORD" \
    V5D_SECRET_INTERNAL_TOKEN="$API_INTERNAL_TOKEN" \
    V5D_SECRET_SESSION="$SESSION_SECRET" \
    V5D_SECRET_OPENAI="$OPENAI_API_KEY" \
    V5D_SECRET_BROWSER_PASSWORD="$BROWSER_PASSWORD" \
    python3 -c '
import os, sys
payload = sys.stdin.read()
for key in (
    "V5D_SECRET_POSTGRES", "V5D_SECRET_MINIO_USER", "V5D_SECRET_MINIO_PASSWORD",
    "V5D_SECRET_INTERNAL_TOKEN", "V5D_SECRET_SESSION", "V5D_SECRET_OPENAI",
    "V5D_SECRET_BROWSER_PASSWORD",
):
    value = os.environ.get(key)
    if value:
        payload = payload.replace(value, "<redacted>")
payload = payload.replace("v5d-browser-", "<redacted-browser-user>-")
...
output.write_text(json.dumps({"schemaVersion": "v5d-built-image-manifest-v1", "images": images}, indent=2) + "\n")
PY
}
...
output.write_text(
    json.dumps({"schemaVersion": "v5d-runtime-container-manifest-v1", "containers": containers}, indent=2)
    + "\n"
)
PY
}
...
worker_run() {
  compose run --rm -T --no-deps --user "$(id -u):$(id -g)" \
    -v "$OUTPUT_DIR:/tmp/v5d-output:rw" \
    -v "$REPO_ROOT/docs/fixtures:/tmp/v5d-fixtures:ro" \
    "$@"
}
...
run_browser() {
  ...
  PLAYWRIGHT_V5D_MIXED_STATE_PATH="$OUTPUT_DIR/browser-state.json" \
  PLAYWRIGHT_V5D_MIXED_ARTIFACT_DIR="$artifact_dir" \
    pnpm --dir "$REPO_ROOT/apps/web" exec playwright test \
      e2e/v5d-mixed-production-start.spec.ts \
...
write_report() {
  python3 - "$OUTPUT_DIR" "$SOURCE_SHA" "$COMPOSE_PROJECT" "$RUN_STATUS" <<'PY'
import json, sys
from pathlib import Path
...
seed = load("seed.json") or {}
before = load("before.json") or {}
after = load("after.json") or {}
verification = load("verification.json") or {}
cleanup = load("cleanup.json") or {}
before_browser = (root / "browser-before/exit-code").read_text().strip() if (root / "browser-before/exit-code").exists() else None
after_browser = (root / "browser-after/exit-code").read_text().strip() if (root / "browser-after/exit-code").exists() else None
state = load("browser-state.json") or {}
# redacted state copy for artifact
if state:
    redacted = dict(state)
    if "password" in redacted:
        redacted["password"] = "<redacted>"
    (root / "browser-state.redacted.json").write_text(json.dumps(redacted, indent=2) + "\n")

empty_after_down = (
    not (root / "containers-after-down.txt").read_text().strip()
    and not (root / "volumes-after-down.txt").read_text().strip()
    and not (root / "networks-after-down.txt").read_text().strip()
) if (root / "containers-after-down.txt").exists() else False

mixed_kinds_ok = set((seed.get("modalities") or [])) == {"pdf", "image", "document"}
semantic_ok = verification.get("passed") is True and before.get("semanticSha256") and before.get("semanticSha256") == after.get("semanticSha256")
browser_ok = before_browser == "0" and after_browser == "0"
deployment_gate = "pass" if (
    run_status == 0 and mixed_kinds_ok and semantic_ok and empty_after_down and browser_ok and cleanup.get("passed") is True
) else ("fail" if run_status != 0 or not semantic_ok else "partial")

report = {
    "schemaVersion": "v5d-mixed-compose-acceptance-v1",
    "sourceSha": source_sha,
    "composeProject": project,
    "modalities": ["pdf", "image", "document"],
    "workspaceId": seed.get("workspaceId") or before.get("workspaceId"),
    "seedModalitiesOk": mixed_kinds_ok,
    "emptyTargetAfterDown": empty_after_down,
    "beforeSemanticSha256": before.get("semanticSha256"),
    "afterSemanticSha256": after.get("semanticSha256"),
    "restoreVerificationPassed": verification.get("passed") is True,
    "browserBeforeExitCode": before_browser,
    "browserAfterExitCode": after_browser,
    "browserRoundtripPassed": browser_ok,
    "deploymentGate": deployment_gate,
    "engineeringGate": "pass" if deployment_gate == "pass" else "fail",
    "releaseGatePassed": deployment_gate == "pass" and cleanup.get("passed") is True,
    "realModelQualityPassed": False,
    "userValuePassed": False,
    "productStage": "internal_preview",
    "cleanup": cleanup,
}
(root / "report.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
print(json.dumps({"deploymentGate": deployment_gate, "releaseGatePassed": report["releaseGatePassed"]}, sort_keys=True))
PY
}
...
if compose ps -q postgres minio redis provider-stub api worker web caddy 2>/dev/null | grep -q .; then
  printf 'v5d_project_already_exists project=%s\n' "$COMPOSE_PROJECT" >&2
  exit 1
fi
OWNS_PROJECT=true

printf 'v5d_mixed_compose_start project=%s output=%s sourceSha=%s\n' "$COMPOSE_PROJECT" "$OUTPUT_DIR" "$SOURCE_SHA"
...
worker_run \
  -e "V5D_BROWSER_EMAIL=$BROWSER_EMAIL" \
  -e "V5D_BROWSER_PASSWORD=$BROWSER_PASSWORD" \
  -e V5D_SEED_API_BASE_URL=http://api:8000 \
  -e "AI_PDF_API_INTERNAL_TOKEN=$API_INTERNAL_TOKEN" \
  worker python scripts/v5d_mixed_deployment_seed.py \
  --state /tmp/v5d-output/browser-state.json \
  --pdf /tmp/v5d-fixtures/evidence-contract/pdf-coordinate-fixture.pdf \
  --image /tmp/v5d-fixtures/evidence-contract/image-coordinate-fixture.png \
  --document /tmp/v5d-fixtures/document-modality/markdown-note.md \
  > "$OUTPUT_DIR/seed.json"

# copy state out is already on bind mount
V5D_WORKSPACE_ID=$(python3 -c 'import json; print(json.load(open("'"$OUTPUT_DIR"'/seed.json"))["workspaceId"])')
...
worker_run \
  -e "V5D_WORKSPACE_ID=$V5D_WORKSPACE_ID" \
  worker python scripts/v5d_mixed_restore_acceptance.py snapshot \
  --mode live --output /tmp/v5d-output/before.json
...
worker_run \
  -e "V5D_WORKSPACE_ID=$V5D_WORKSPACE_ID" \
  worker python scripts/v5d_mixed_restore_acceptance.py snapshot \
  --mode live --output /tmp/v5d-output/after.json
worker_run \
  -e "V5D_WORKSPACE_ID=$V5D_WORKSPACE_ID" \
  worker python scripts/v5d_mixed_restore_acceptance.py verify \
  --before /tmp/v5d-output/before.json \
  --after /tmp/v5d-output/after.json \
  --output /tmp/v5d-output/verification.json
...
RUN_STATUS=0
printf 'v5d_mixed_compose_pass project=%s workspace=%s\n' "$COMPOSE_PROJECT" "$V5D_WORKSPACE_ID"
