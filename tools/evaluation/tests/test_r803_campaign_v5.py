...
import citeframe_evaluation.provider as evaluation_provider
import pytest
from ai_pdf_api.services.evaluation import parse_evaluation_report
from citeframe_evaluation.paired import run_paired_evaluation
...
from citeframe_evaluation.runtime.quick import run_quick_case, run_quick_case_with_diagnostics
...
from ai_pdf_worker.research.schemas import validate_agent_result
...
def test_package_v5_loads_with_threshold_and_scorer_v2() -> None:
    ...
    assert plan.plan_document["evaluatorClosureRoots"] == [
        "citeframe_evaluation.campaign"
    ]
    modules = plan.evaluator_closure_modules
    expected_needles = [
        "tools/evaluation/src/citeframe_evaluation/paired.py",
        "tools/evaluation/src/citeframe_evaluation/policy.py",
        "apps/worker/src/ai_pdf_worker/research/executor.py",
        "apps/worker/src/ai_pdf_worker/research/engine.py",
        "apps/worker/src/ai_pdf_worker/research/tools.py",
        "apps/worker/src/ai_pdf_worker/research/core.py",
        "apps/worker/src/ai_pdf_worker/research/adapters/generation.py",
        "apps/worker/src/ai_pdf_worker/research/adapters/ledger.py",
        "apps/worker/src/ai_pdf_worker/research/agents.py",
...
def test_closure_required_import_missing_module_raises(tmp_path: Path) -> None:
    from citeframe_evaluation.integrity import compute_evaluator_closure
...
def test_recursive_evaluator_closure_hash_changes_with_transitive_module(
    ...
    assert "tools/evaluation/src/citeframe_evaluation/policy.py" in baseline["modules"]

    # Copy a transitive critical module into an isolated tree and mutate only that copy.
    relative = "tools/evaluation/src/citeframe_evaluation/policy.py"
...
def test_round_root_symlink_and_non_directory_are_rejected(tmp_path: Path) -> None:
    package = load_evaluation_package(DEFAULT_PACKAGE_V5_PATH)
    plan = freeze_campaign_plan(package)
    from citeframe_evaluation.campaign.storage import _write_immutable_json
...
def test_raw_output_manifest_unique_paths_hashes_and_secret_scan(tmp_path: Path) -> None:
    ...
    from citeframe_evaluation.contracts import canonical_sha256
...
def test_quick_diagnostic_capture_uses_succeeded_provider_attempt_number(
    ...
    from citeframe_evaluation.provider import ProviderResult
...
def test_freeze_interruption_requires_valid_partial_start_companion(
    ...
    package = load_evaluation_package(DEFAULT_PACKAGE_V5_PATH)
    plan = freeze_campaign_plan(package)
    from citeframe_evaluation.campaign.storage import _write_immutable_json, _write_round_start_marker
...
def test_round_interruption_freezes_terminal_and_preserves_partial(
    ...
    monkeypatch.setattr(
        "citeframe_evaluation.campaign.rounds.run_quick_case_with_diagnostics",
        boom,
    )
...
def test_semantic_failures_bind_exact_logical_call_and_raw_sha() -> None:
    from citeframe_evaluation.diagnostics import DiagnosticCapture
    from citeframe_evaluation.runtime.failures import _diagnostic_for_semantic_failure
...
def test_runtime_branch_validation_diagnostic_uses_exact_researcher_key(
    ...
    monkeypatch.setattr(
        "ai_pdf_worker.research.tools.EvidenceToolRegistry.validate_branch_result",
        patched_validate,
    )
...
def test_round_start_is_durable_before_first_provider_generate(
    ...
    plan = freeze_campaign_plan(package)
    events: list[str] = []
    import citeframe_evaluation.campaign.storage as campaign_mod
...
def test_v4_public_wrapper_unknown_exception_keeps_type_name(
    ...
    monkeypatch.setattr(
        "citeframe_evaluation.provider.EvaluationGeneration.generate",
        boom,
    )
    quick = run_quick_case(package, package.cases[0], provider)
    research = run_research_case(package, package.cases[0], provider)
    ...
    from citeframe_evaluation.diagnostics import classify_failure_origin
    from citeframe_evaluation.scoring import score_case_v2
...
def test_worker_docs_use_repo_root_uv_invocation() -> None:
    ...
    assert "uv run --project tools/evaluation" in text
    # Command form must work from repo root (documented cwd).
    assert "python -m citeframe_evaluation.cli.campaign" in text
