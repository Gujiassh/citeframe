...
def investigation_report(db, run_id, *, fact_claims, unresolved_claims):
    ...
    if outcome is None:
        return canonical_final_report(
            fact_claims=fact_claims, unresolved_claims=unresolved_claims
        )
    ...
    unresolved = [
        ...
        if not outcome["resolved"] or c.id not in original_ids
    ]
    text = canonical_final_report(
        fact_claims=fact_claims, unresolved_claims=unresolved
    ).decode()
    lines = [
        text.rstrip(),
        "",
        ...
        "Stop reason: " + clean(outcome["reason"]),
        clean(outcome["explanation"]),
    ]
    lines += ["", "### Original conclusions"]
    for original in outcome["originalClaims"]:
        lines += [
            "- " + clean(original["text"]) + " (" + original["id"] + ")",
            "  Evidence: " + ", ".join(original["evidenceHandleIds"]),
        ]
    for revision in outcome["revisions"]:
        lines += [
            "",
            "- " + clean(revision["text"]),
            "  Original claims: " + ", ".join(revision["originalClaimIds"]),
    ...
    for item in outcome["inspections"]:
        lines += [
            "- "
            + item["evidenceHandleId"]
            + ": "
            + "; ".join(
                k + "=" + clean(item[k] if item[k] is not None else "unknown")
    ...
    if outcome["queries"]:
        lines += [
            "",
            "### Queries checked",
            *["- " + clean(q) for q in outcome["queries"]],
        ]
    if outcome["gaps"]:
        lines += [
            "",
            "### Important gaps",
            *["- " + clean(g) for g in outcome["gaps"]],
            "",
...
def render_final_report(db, snapshot, *, fact_claims, unresolved_claims):
