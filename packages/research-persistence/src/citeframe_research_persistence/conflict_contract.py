...
INVESTIGATOR_SCHEMA = {
    ...
    "required": ["inspections", "revisions", "gaps", "reason", "nextQuery"],
    "properties": {
        "inspections": {
            "type": "array",
            "minItems": 1,
        ...
        "revisions": {
            ...
            "items": {
                ...
                "properties": {
                    "originalClaimIds": _IDS,
                    "text": _TEXT,
                    "evidenceHandleIds": _IDS,
                },
            },
        },
        "gaps": {"type": "array", "maxItems": 30, "items": _TEXT},
        "reason": _TEXT,
...
def validate_investigation(value, *, claims=None, evidence=None):
    ...
    for item in value["inspections"]:
        ...
        if (
            handle is None
            or item["evidenceHandleId"] in inspected
            or item["quote"] not in handle["excerpt"]
        ):
    ...
    for revision in value["revisions"]:
        if not set(revision["originalClaimIds"]).issubset(source) or not set(
            revision["evidenceHandleIds"]
        ).issubset(inspected):
