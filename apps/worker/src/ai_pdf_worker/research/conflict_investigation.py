...
def investigate(*, execution, state, agents, lease, tools, checkpoint):
    ...
    unchanged = [
        ...
        if c.id not in conflicts and c.verification_status == "supported"
    ]
    claims = [
        {"id": c.id, "text": c.text, "evidenceHandleIds": list(c.evidence_handle_ids)}
        for c in originals
    ]
    ...
    try:
        for turn in range(3):
            payload = {
                "claims": claims,
                "evidence": [asdict(e) for e in evidence.values()],
                "remainingSearches": 2 - len(searches),
                "previousInspections": inspections,
                "gaps": gaps,
            }
            result = perform(
                "inspect", payload, lambda: agents.investigator(payload, lease)
            )
            if result.get("stopped"):
                reason = result["stopped"]
                break
            validate_investigation(
                result, claims=claims, evidence=[asdict(e) for e in evidence.values()]
            )
            inspections, gaps = result["inspections"], result["gaps"]
            explanation = result["reason"]
            revisions = [
                {**v, "id": str(uuid5(NAMESPACE_URL, f"{lease.step_id}:{turn}:{i}"))}
                for i, v in enumerate(result["revisions"])
            ]
            if revisions:
                drafts = [
                    DraftClaim(v["id"], v["text"], tuple(v["evidenceHandleIds"]))
                    for v in revisions
                ]
                ...
                if len(rows) != len(drafts) or set(by_id) != {c.id for c in drafts}:
                    raise ResearchExecutionError(
                        "investigation_verifier_claim_set_mismatch"
                    )
                for draft in drafts:
                    c = by_id[draft.id]
                    if (
                        c.text != draft.text
                        or tuple(c.evidence_handle_ids) != draft.evidence_handle_ids
                        or c.verification_status not in {"supported", "unsupported"}
                        or c.conflict_status != "none"
                    ):
                        raise ResearchExecutionError(
                            "investigation_verifier_mutated_claim"
                        )
                if all(c.verification_status == "supported" for c in rows):
                    check_claims = [*unchanged, *rows]
                    critique = perform(
                        "critic",
                        {"claims": [asdict(c) for c in check_claims]},
                        lambda: {
                            "conflictClaimIds": list(agents.critic(check_claims, lease))
                        },
                    )
                    if critique.get("stopped"):
                        reason = critique["stopped"]
                        break
                    ids = critique["conflictClaimIds"]
                    if len(ids) != len(set(ids)) or not set(ids).issubset(
                        {c.id for c in check_claims}
                    ):
                        raise ResearchExecutionError(
                            "investigation_critic_scope_invalid"
                        )
                    if not ids:
                        resolved = True
                        reason = "source_backed_revision"
                        break
            ...
            request = {
                "query": query,
                "assetIds": [a.asset_id for a in execution.frozen_assets],
                "topK": execution.retrieval_top_k,
            }

            def search():
                search_tools = tools(len(searches))
                found = search_tools.search(
                    query=query, asset_ids=request["assetIds"], top_k=request["topK"]
                )
                if found:
            ...
            if found.get("stopped"):
                reason = found["stopped"]
                break
            existing = {key(e) for e in evidence.values()}
            fresh = [
                EvidenceHandle(**v)
                for v in found["evidence"]
                if key(EvidenceHandle(**v)) not in existing
            ]
            if not fresh:
    ...
    outcome = {
        ...
        "queries": searches,
        "gaps": gaps,
        "revisions": revisions if resolved else [],
        "evidence": [asdict(e) for e in evidence.values()],
    }
    if not resolved and not gaps:
        outcome["gaps"] = [
            "The cited sources do not establish a verified, mutually consistent correction; do not treat this result as a complete operating checklist."
        ]
    # The final record is persisted before the existing atomic gate completion.
    saved = checkpoint(
        lease, operation, "finish", {"conflictClaimIds": sorted(conflicts)}
    )
    if saved["status"] == "succeeded":
        ...
        checkpoint(
            lease, operation, "finish", {"conflictClaimIds": sorted(conflicts)}, outcome
        )
    return outcome
