...
def upgrade():
    with op.batch_alter_table("human_decisions") as batch:
        batch.add_column(
            sa.Column(
                "decision_origin", sa.String(16), nullable=False, server_default="human"
            )
        )
        batch.create_check_constraint(
            "ck_human_decisions_origin", "decision_origin IN ('human','policy')"
        )
    ...
    data = json.loads(
        (Path(__file__).parents[1] / "release_data/research_v3.json").read_text(
            encoding="utf-8"
        )
    )
    bind = op.get_bind()
...
def downgrade():
    raise RuntimeError(
        "Research policy decisions and frozen v3 executions require a non-lossy forward migration."
    )
