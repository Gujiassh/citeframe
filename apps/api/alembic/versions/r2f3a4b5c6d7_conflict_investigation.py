...
def upgrade():
    op.create_table("research_conflict_turns",
        ...
        sa.Column("phase", sa.String(16), nullable=False), sa.Column("status", sa.String(16), nullable=False),
        sa.Column("request_sha256", sa.String(64), nullable=False), sa.Column("request_json", sa.JSON(), nullable=False),
        sa.Column("result_sha256", sa.String(64)), sa.Column("result_json", sa.JSON()),
        sa.CheckConstraint("operation_number >= 0 AND operation_number < 13", name="ck_conflict_operation_bound"),
        sa.CheckConstraint("status IN ('started','succeeded')", name="ck_conflict_operation_status"),
        sa.CheckConstraint("phase IN ('inspect','search','verify','critic','finish')", name="ck_conflict_operation_phase"))
    import hashlib
    import json
    from pathlib import Path
    data = json.loads((Path(__file__).parents[1] / "release_data/research_v4.json").read_text(encoding="utf-8"))
    bind = op.get_bind()
    meta = sa.MetaData()
    ...
    bind.execute(workflow.insert().values(id=data["workflowId"], workflow_key="evidence_research",
        version_number=4, availability="active", manifest_schema_version="2",
    ...
    for node, spec in data["prompts"].items():
        bind.execute(prompt.insert().values(id=spec["id"], prompt_key=spec["prompt_key"],
            version_number=4, step_kind=spec["step_kind"], availability="active",
