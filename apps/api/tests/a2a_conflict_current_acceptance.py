...
from ai_pdf_api.services.research.research_prompt_provenance import V3_WORKFLOW_VERSION_ID, V3_PROMPT_VERSION_IDS, V4_WORKFLOW_VERSION_ID, V4_PROMPT_VERSION_IDS
...
def assert_v3_restored(rows, objects, api_payload_bytes, workflow, source):
    ...
    for key in ("workflowId", "workflowVersion", "releaseId", "manifestSha256", "agentResultSchemaVersion", "prompts"):
...
def assert_v4_completion(rows, objects, api_payload_bytes, workflow):
    assert workflow["workflowId"] == workflow["currentDefaultWorkflowId"] == V4_WORKFLOW_VERSION_ID
    ...
    assert workflow["agentResultSchemaVersion"] == workflow["currentDefaultAgentSchema"] == "research-agent-results-v3"
    ...
    gate=next(s for s in rows["research_steps"] if s["step_kind"]=="conflict_decision_gate")
    attempts={a["id"]:a for a in rows["research_step_attempts"]}
    assert all(t["step_id"]==gate["id"] and t["execution_snapshot_id"]==gate["execution_snapshot_id"]
        and attempts[t["created_by_attempt_id"]]["step_id"]==gate["id"]
        and attempts[t["created_by_attempt_id"]]["status"]=="succeeded" for t in turns)
    def decode(v):return json.loads(v) if isinstance(v,str) else v
    for t in turns:
    ...
    for evidence in outcome["evidence"]:
        ...
        assert evidence["source_fingerprint_sha256"]==source["source_fingerprint_sha256"]
        assert handle["execution_snapshot_id"]==gate["execution_snapshot_id"]
    assert outcome["queries"]==[" ".join(decode(turns[1]["request_json"])["query"].split()).casefold()]
    assert all(i[k] is None for i in outcome["inspections"] for k in ("version", "environment", "time", "conditions"))
    assert {c["id"] for c in outcome["originalClaims"]} == {c["id"] for c in rows["research_claims"]}
    projected=deepcopy(workflow)
    projected.update(workflowId=V3_WORKFLOW_VERSION_ID,currentDefaultWorkflowId=V3_WORKFLOW_VERSION_ID,workflowVersion=3,
    ...
    assert_current_completion(rows, objects, api_payload_bytes, projected)
    artifact=next(a for a in rows["research_artifacts"] if a["artifact_kind"]=="final_report")
